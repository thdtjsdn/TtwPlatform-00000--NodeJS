//----------------------------------------------------------------------------------------------------;
var fileNm = "js-UncompressSource/router_internal/_source/uncompress/json.js";
...
(function( req, res ){
	...
	global.b2link.router_file.post_ReceiveData__String( req, res, function( req, res, data ){
		...
		try
		{
			//JavaScript Code를 UnCompress 한다.;
			srcMin = global.uncompressCSSJSHTML.json_source( data );
		}
		catch( e )
		{
			...
			//UnCompress 결과가 Error 일시 처리 및 Client에 전송;
			global.b2link.response.send_200_JSON( req, res, e );
			return;
		}

		//UnCompress Code 실행 후 결과를 Client에 전송하기;
