//----------------------------------------------------------------------------------------------------;
var fileNm = "js-CompressJS/router_internal/_source/compress/json.js";
 ...
 * @example
 * <code>
	http://localhost:port/_source/compress/json?data=var a = 1;
 * </code>
 */
(function( req, res ){
	//global.TtwLog.log( "- [ S ] - _source/compress/json():void----------" );
	...
	//if( !global.compressJS ) global.b2link.compressJS.install();
	...
	global.b2link.router_file.post_ReceiveData__String( req, res, function( req, res, data ){
		...
		try
		{
			...
			//JSON Code를 Minify 한다.;
			srcMin = JSON.stringify( o );
		}
		catch( e )
		{
	...
	//global.TtwLog.log( "- [ E ] - _source/compress/js():void----------" );
});